...
with open(os.path.join(EXPERIMENT_DIR, "..", "mm79", "data", "normal_ranges.yaml")) as f:
    RANGES = yaml.load(f, Loader=yaml.FullLoader)
...
def plot_tree(decision_tree, max_depth=None, feature_names=None,
              ...
              precision=3, ax=None, fontsize=None, feat_map=None, dataset=None):
    ...
    exporter = _MPLTreeExporter(
        ...
        precision=precision, fontsize=fontsize, feat_map=feat_map, dataset=dataset)
...
class _BaseTreeExporter(object):
    def __init__(self, max_depth=None, feature_names=None,
                 ...
                 impurity=True, node_ids=False,
                 proportion=False, rotate=False, rounded=False,
                 precision=3, fontsize=None, feat_map=None, dataset=None):
    ...
    def node_to_str(self, tree, node_id, criterion):
        ...
        if tree.children_left[node_id] != _tree.TREE_LEAF:
            ...
            if self.feat_map is not None:
                feature_code = self.feat_map[feature]
                if feature_code[0] == 'x':
                    og_feat_name = feature_code[2]
                    unnormalized_threshold = self.dataset.unnormalize_single_X_feat(
                        tree.threshold[node_id], og_feat_name, RANGES)
                elif feature_code[0] == 'b':
                    unnormalized_threshold = self.dataset.unnormalize_single_B(
                        tree.threshold[node_id], feature_code[1])
                elif feature_code[0] == 't':
                    unnormalized_threshold = tree.threshold[node_id]

                node_string += '%s %s %s%s' % (feature,
                                               characters[3],
                                               round(unnormalized_threshold,
                                                     self.precision),
                                               characters[4])
            else:
                node_string += '%s %s %s%s' % (feature,
...
class _MPLTreeExporter(_BaseTreeExporter):
    def __init__(self, max_depth=None, feature_names=None,
        ...
        super().__init__(
            ...
            rotate=rotate, rounded=rounded, precision=precision, feat_map=feat_map, dataset=dataset)
        self.fontsize = fontsize
